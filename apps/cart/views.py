...
class AddItemView(APIView):
    def post(self, request, format=None):
        ...
        try:
            ...
            if CartItem.objects.filter(cart=cart, product=product, size=size).exists():
                return Response(
            ...
            if int(product.quantity) > 0:
                CartItem.objects.create(
                    product=product, cart=cart, count=count, size = size
                )
...
class UpdateItemView(APIView):
    def put(self, request, format=None):
        ...
        try:
            ...
            return Response(
                {'error': 'Size of product must be valid'},
                status=status.HTTP_404_NOT_FOUND)
        ...
        try:
            ...
            if not CartItem.objects.filter(cart=cart, product=product, size=size).exists():
                return Response(
            ...
            if count <= quantity:
                CartItem.objects.filter(
                    product=product, cart=cart, size = size
                ).update(count=count)
            ...
            for cart_item in cart_items:
                ...
                if CartItem.objects.filter(cart=cart, product=product, size = size).exists():
                    ...
                    if cart_item_count <= quantity:
                        CartItem.objects.create(
                            product=product, cart=cart, count=cart_item_count, size = size
                        )
